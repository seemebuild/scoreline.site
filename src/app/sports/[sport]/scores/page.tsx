import type { Metadata } from "next";
import { notFound } from "next/navigation";

import { SiteShell } from "../../../components/site-shell";
import { launchSports } from "../../../../domain/sports/public-catalog";
import { buildSportPath } from "../../../../domain/sports/routing";
import { createJobsPrismaClient } from "../../../../domain/jobs/client";
import { getSoccerLiveScoreRows } from "../../../../domain/sports/public-data";

type SportPageParams = {
  sport: string;
};

export function generateStaticParams(): SportPageParams[] {
  return launchSports.map((sport: (typeof launchSports)[number]) => ({ sport: sport.slug }));
}

export function generateMetadata({ params }: { params: SportPageParams }): Metadata {
  const sport = launchSports.find((item: (typeof launchSports)[number]) => item.slug === params.sport);
  if (!sport) return {};

  return {
    title: `${sport.name} live scores`,
    description: `Current live scores and match status for ${sport.name}.`,
    alternates: {
      canonical: `${buildSportPath(sport.slug)}/scores`,
    },
  };
}

export default async function SportScoresPage({ params }: { params: SportPageParams }) {
  const sport = launchSports.find((item: (typeof launchSports)[number]) => item.slug === params.sport);
  if (!sport) notFound();

  if (sport.slug !== "soccer") {
    return (
      <SiteShell>
        <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
          <h1 className="text-4xl font-black">{sport.name} live scores</h1>
          <p className="mt-4 max-w-2xl text-lg leading-8 text-[color:var(--muted)]">
            Live scores for this sport will come once the launch soccer path is complete.
          </p>
        </section>
      </SiteShell>
    );
  }

  const prisma = createJobsPrismaClient();
  const scores = await getSoccerLiveScoreRows(prisma);

  return (
    <SiteShell>
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        <h1 className="text-4xl font-black">{sport.name} live scores</h1>
        <div className="mt-6 grid gap-3">
          {scores.length === 0 ? (
            <p className="text-[color:var(--muted)]">No live soccer matches are active right now.</p>
          ) : (
            scores.map((score) => (
              <article className="border border-[color:var(--line)] bg-[color:var(--surface)] p-4" key={score.id}>
                <p className="text-sm font-bold text-[color:var(--accent-strong)]">{score.competitionName}</p>
                <h2 className="mt-1 text-xl font-black">{score.title}</h2>
                <p className="mt-1 text-sm text-[color:var(--muted)]">
                  {score.homeScore} - {score.awayScore}
                </p>
              </article>
            ))
          )}
        </div>
      </section>
    </SiteShell>
  );
}
